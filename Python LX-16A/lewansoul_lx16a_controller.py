...
class ServoController(object):
    ...
    def _command(self, command, *params):
        length = 2 + len(params)
        with self._lock:
            LOGGER.debug('Envoi vers le servo, le paquet : %s', hex_data([
    ...
    def _wait_for_response(self, command, timeout=None):
        ...
        while True:
            ...
            if data[-1] != 0x55:
                LOGGER.error(
                    'Octet inattendu obtenu en attendant l en-tête de la réponse: %02x',
                    data[-1]
                )
                continue
            data += read(1)
            ...
            LOGGER.debug('Réponse à la commande %s: %s', cmd, hex_data(data))
    ...
    def move(self, positions, time=0):
        """Commander plusieurs servos pour se déplacer à des positions données dans un temps donné"""
    ...
    def get_positions(self, servo_ids):
        """Lit les positions des servos avec les identifiants donnés et renvoie une carte
         de l'identifiant d'asservissement à la position correspondante.
        """
    ...
    def unload(self, servo_ids):
        """Eteint les moteurs des servos avec des ID donnés"""
        self._command(CMD_MULT_SERVO_UNLOAD, len(servo_ids), *servo_ids)

    def get_battery_voltage(self):
        """Renvoie le niveau de tension d'alimentation du servo-contrôleur en millivolts"""
