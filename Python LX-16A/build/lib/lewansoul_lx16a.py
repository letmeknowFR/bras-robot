...
class ServoController(object):
    ...
    def _wait_for_response(self, servo_id, command, timeout=None):
        ...
        while True:
            ...
            cmd = data[4]
            if length > 7:
                LOGGER.error('Longueur invalide pour le paquet %s', list(data))
                continue
            ...
            if 255-(sid + length + cmd + sum(params)) % 256 != checksum:
                LOGGER.error('Checksum invalide pour le paquet %s', list(data))
                continue

            if cmd != command:
                LOGGER.warning('Réponse inattendu de %s pour %s',
                               cmd, list(data))
                continue

            if servo_id != SERVO_ID_ALL and sid != servo_id:
                LOGGER.warning('Obtention d une commande du servo %s', sid)
                continue
    ...
    def get_prepared_move(self, servo_id, timeout=None):
        """Retourne la position du servo et le temps"""
